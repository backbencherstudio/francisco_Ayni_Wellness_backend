...
export class MoodService {
  ...
  private validateEmotions(emotions: string[]) {
    if (!Array.isArray(emotions))
      throw new BadRequestException('Emotions must be an array');
    const canon = new Set(EMOTION_KEYS);
    for (const raw of emotions) {
      const key = this.normalizeEmotion(raw);
      if (!canon.has(key))
        throw new BadRequestException(`Unknown emotion: ${raw}`);
    }
  ...
  private classify(score: number, emotions: string[]) {
    ...
    const positiveSet = new Set(
      EMOTIONS.filter((e) => e.valence === 'positive').map((e) => e.key),
    );
    const negativeSet = new Set(
      EMOTIONS.filter((e) => e.valence === 'negative').map((e) => e.key),
    );

    let pos = 0,
      neg = 0;
    for (const e of emotions.map((x) => this.normalizeEmotion(x))) {
      if (positiveSet.has(e)) pos++;
      else if (negativeSet.has(e)) neg++;
    }
    ...
    if (delta >= 2 && idx < order.length - 1) idx++;
    else if (delta <= -2 && idx > 0) idx--;
  ...
  async getTrend(userId: string, days = 7) {
    ...
    if (!aggregates.length)
      return { success: true, range_days: days, avg_score: null, entries: [] };
    const avg =
      aggregates.reduce((s, a) => s + a.avg_score, 0) / aggregates.length;
    return {
      success: true,
      range_days: days,
  ...
  async insights(userId: string, days = 7) {
    ...
    const avgRecent =
      scoreListRecent.reduce((s, n) => s + n, 0) / scoreListRecent.length;
    const avgPrev = scoreListPrev.length
      ? scoreListPrev.reduce((s, n) => s + n, 0) / scoreListPrev.length
      : null;
    const delta = avgPrev != null ? avgRecent - avgPrev : null;
    const variance =
      scoreListRecent.reduce((s, n) => s + Math.pow(n - avgRecent, 2), 0) /
      scoreListRecent.length;
    ...
    for (const e of entriesRecent)
      for (const emo of e.emotions) freq[emo] = (freq[emo] || 0) + 1;
    const sorted = Object.entries(freq).sort((a, b) => b[1] - a[1]);
    const topEmotions = sorted
      .slice(0, 5)
      .map(([k, v]) => ({ emotion: k, count: v }));
    ...
    for (const [emo, count] of sorted) {
      ...
      if (positiveSet.has(key)) pos += count;
      else if (negativeSet.has(key)) neg += count;
    }
    ...
    const summary = this.buildInsightSummary({
      avgRecent,
      delta,
      stddev,
      negativeRatio,
    ...
    const suggestion = this.buildInsightSuggestion({
      avgRecent,
      stddev,
      negativeRatio,
      pos,
  ...
  private buildInsightSummary(params: {
    avgRecent: number;
    delta: number | null;
    stddev: number;
    ...
    const deltaPart =
      delta == null
        ? ''
        : delta > 0
          ? `up ${delta.toFixed(1)} vs previous period`
          : delta < 0
            ? `down ${Math.abs(delta).toFixed(1)} vs previous period`
  ...
  private buildInsightSuggestion(params: {
    avgRecent: number;
    stddev: number;
    negativeRatio: number;
    pos: number;
    ...
    if (avgRecent >= 8 && negativeRatio < 0.25)
      return 'Your mood is strong and consistent—keep reinforcing the routines that support this balance.';
    if (avgRecent >= 6 && negativeRatio < 0.35)
      return 'Mood is generally healthy. Consider a brief daily reflection to nudge it even higher.';
    if (avgRecent < 6 && negativeRatio >= 0.4)
      return 'Focus on one small uplifting habit (short walk, gratitude note) to counter repeated low emotions.';
    if (stddev >= 2)
      return 'Mood swings are higher than normal—try tracking context (sleep, activities) to spot triggers.';
  ...
  async update(userId: string, id: string, dto: UpdateMoodDto) {
    const prismaAny: any = this.prisma as any;
    const existing = await prismaAny.moodEntry.findFirst({
  ...
  async remove(userId: string, id: string) {
    ...
    if (!existing) throw new NotFoundException('Mood not found');
    await prismaAny.moodEntry.update({
      where: { id },
