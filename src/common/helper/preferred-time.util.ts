...
export function normalizePreferred(
  ...
  if (lowMap[low]) return lowMap[low];
...
export function generatePreferredSlots(
...
export function validateReminderAgainstPreferred(
  reminder: string,
  pref?: $Enums.PreferredTime,
) {
  if (!pref) return; 
  ...
  if (!match)
    throw new BadRequestException(
      'reminder_time must be HH:MM or HH:MM:SS (optionally inside ISO datetime)',
    );
  ...
  if (h < win.start || h >= win.end || (h === win.end && m > 0)) {
    throw new BadRequestException(
      `reminder_time must fall within ${pref} window`,
    );
  }
}
...
export function getReminderSlots(preferredRaw: string) {
  ...
  return {
    success: true,
    preferred_time: pref,
    preferred_time_label: uiLabelMap[pref],
    slots,
    format: { ui: 'label', value: 'HH:MM', stored: 'HH:MM:SS' },
  };
}
